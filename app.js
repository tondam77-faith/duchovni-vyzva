...
if (!localStorage.getItem('exodus_logs')) { localStorage.setItem('exodus_logs', JSON.stringify([])); }
...
function loadData() { return { habits: JSON.parse(localStorage.getItem('exodus_habits')), logs: JSON.parse(localStorage.getItem('exodus_logs')) }; }
function saveData(habits, logs) { localStorage.setItem('exodus_habits', JSON.stringify(habits)); localStorage.setItem('exodus_logs', JSON.stringify(logs)); }
...
function renderTodayView() {
    ...
    const activeHabits = habits.filter(h => h.isActive && (!h.daysOfWeek || h.daysOfWeek.includes(todayDayOfWeek)));
    ...
    activeHabits.forEach(habit => {
        ...
        card.innerHTML = `<input type="checkbox" ${isDone ? 'checked' : ''} onchange="toggleHabitToday('${habit.id}')">
            <div class="habit-info"><p class="habit-title">${habit.name}</p><p class="habit-trigger">${habit.trigger}</p></div>`;
...
function toggleHabitToday(habitId) {
    ...
    if (todayLogIndex === -1) { logs.push({ date: todayStr, completedHabitIds: [], eveningReflection: '', gratitude: '' }); todayLogIndex = logs.length - 1; }
    const completedIds = logs[todayLogIndex].completedHabitIds;
    const idIndex = completedIds.indexOf(habitId);
    if (idIndex === -1) { completedIds.push(habitId); } else { completedIds.splice(idIndex, 1); }
...
function renderSettingsView() {
    ...
    habits.forEach(habit => {
        ...
        if (habit.daysOfWeek && habit.daysOfWeek.length > 0 && habit.daysOfWeek.length < 7) {
            ...
            daysStr = [...habit.daysOfWeek].sort((a,b) => (a===0?7:a) - (b===0?7:b)).map(d => dayNames[d]).join(', ');
        }
        ...
        item.innerHTML = `
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <button class="btn-toggle-status ${habit.isActive ? '' : 'paused'}" onclick="toggleHabitStatus('${habit.id}')">${habit.isActive ? 'Uspat' : 'Aktivovat'}</button>
            </div>
        `;
...
function editHabit(habitId) {
    ...
    if (habit) {
        ...
        checkboxes.forEach(cb => { cb.checked = activeDays.includes(parseInt(cb.value)); });
        editingHabitId = habit.id;
        document.getElementById('btn-save-habit').innerText = 'Uložit úpravy';
...
function saveHabit() {
    ...
    const selectedDays = Array.from(document.querySelectorAll('.day-checkbox')).filter(cb => cb.checked).map(cb => parseInt(cb.value));
    const finalDays = selectedDays.length > 0 ? selectedDays : [0,1,2,3,4,5,6];

    if (!name || !trigger) { alert('Prosím, vyplň název návyku i jeho konkrétní spouštěč.'); return; }
    
    const { habits, logs } = loadData();
    if (editingHabitId) {
        const index = habits.findIndex(h => h.id === editingHabitId);
        if (index !== -1) { habits[index] = { ...habits[index], name, category, trigger, daysOfWeek: finalDays }; }
        alert('Tvé úpravy byly laskavě uloženy.');
    } else {
        habits.push({ id: String(Date.now()), name, category, trigger, isActive: true, daysOfWeek: finalDays });
...
function toggleHabitStatus(habitId) {
    const { habits, logs } = loadData();
    const habit = habits.find(h => h.id === habitId);
    if (habit) { habit.isActive = !habit.isActive; saveData(habits, logs); renderSettingsView(); }
}

function exportData() {
    const data = { habits: JSON.parse(localStorage.getItem('exodus_habits') || '[]'), logs: JSON.parse(localStorage.getItem('exodus_logs') || '[]') };
    ...
    reader.onload = function(e) {
        try {
            ...
            if (data.habits && data.logs) {
                ...
                location.reload(); 
            } else { alert('Zvolený soubor neobsahuje platná data.'); }
...
function renderCalendarView() {
    ...
    for (let i = 0; i < startOffset; i++) {
        const emptyBox = document.createElement('div'); emptyBox.className = 'calendar-day-box empty'; grid.appendChild(emptyBox);
    }
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        const expectedHabitsCount = habits.filter(h => h.isActive && (!h.daysOfWeek || h.daysOfWeek.includes(checkDayOfWeek))).length;
        ...
        if (checkDate > todayDate) {
            ...
            if (log && (log.completedHabitIds.length > 0 || log.eveningReflection)) {
                ...
                if (checkDate < todayDate) { dayBox.classList.add('none-done'); }
            }
...
function showDayDetail(dateStr) {
    ...
    const activeHabitsForDay = habits.filter(h => h.isActive && (!h.daysOfWeek || h.daysOfWeek.includes(checkDayOfWeek)));
    const completedIds = log ? log.completedHabitIds : [];
    
    const completedHeader = document.createElement('h4'); completedHeader.innerText = 'To, co se podařilo:'; completedHeader.style.color = 'var(--success-color)'; completedHeader.style.margin = '16px 0 6px 0'; habitsDiv.appendChild(completedHeader);
    
    let hasCompleted = false;
    habits.forEach(h => {
        if (completedIds.includes(h.id)) {
            hasCompleted = true;
            const p = document.createElement('p'); p.style.margin = '4px 0'; p.innerHTML = `✓ <strong>${h.name}</strong> <span style="font-size:0.8rem; color:var(--text-muted);">(${h.category})</span>`; habitsDiv.appendChild(p);
        }
    });
    
    if (!hasCompleted) { habitsDiv.innerHTML += '<p style="color:var(--text-muted); font-style:italic; margin:4px 0;">V tento den nebyl splněn žádný návyk.</p>'; }
    
    if (activeHabitsForDay.length > 0) {
        const uncompletedHeader = document.createElement('h4'); uncompletedHeader.innerText = 'To, co dnes nevyšlo (prostor pro reflexi):'; uncompletedHeader.style.color = 'var(--text-muted)'; uncompletedHeader.style.margin = '20px 0 6px 0'; habitsDiv.appendChild(uncompletedHeader);
        let hasUncompleted = false;
        activeHabitsForDay.forEach(h => {
            if (!completedIds.includes(h.id)) {
                hasUncompleted = true;
                const p = document.createElement('p'); p.style.margin = '4px 0'; p.style.color = 'var(--text-muted)'; p.innerHTML = `• <span style="text-decoration: line-through; opacity: 0.6;">${h.name}</span>`; habitsDiv.appendChild(p);
            }
        });
        if (!hasUncompleted) { habitsDiv.innerHTML += '<p style="color:var(--success-color); font-style:italic; margin:4px 0;">Všechny tehdejší aktivní návyky byly splněny!</p>'; }
    } else {
        habitsDiv.innerHTML += '<p style="color:var(--text-muted); margin-top:20px;">V tento den nebyl v plánu žádný návyk.</p>';
    }
    
    document.getElementById('detail-reflection').innerHTML = log && log.eveningReflection ? `<div style="margin-top:16px; padding:10px; background:var(--bg-color); border-left:3px solid var(--border-color); border-radius:4px; color:var(--text-main);"><strong>Moje tehdejší boje:</strong><br>${log.eveningReflection}</div>` : '';
    document.getElementById('detail-gratitude').innerHTML = log && log.gratitude ? `<div style="margin-top:10px; padding:10px; background:var(--success-bg); border-left:3px solid var(--success-color); border-radius:4px; color:var(--text-main);"><strong>Vděčnost:</strong><br>${log.gratitude}</div>` : '';
}
